...
DECLARE
    CURSOR FIDE_DIRECCION_PROVINCIA_TB IS SELECT ID_PROVINCIA, NOMBRE FROM FIDE_DIRECCION_PROVINCIA_TB;
    V_ID_PROVINCIA FIDE_DIRECCION_PROVINCIA_TB.ID_DIRECCION_PROVINCIA_PK%TYPE;
    V_NOMBRE FIDE_DIRECCION_PROVINCIA_TB.NOMBRE_PROVINCIA%TYPE;
BEGIN
    OPEN FIDE_DIRECCION_PROVINCIA_TB;
    LOOP
        FETCH FIDE_DIRECCION_PROVINCIA_TB INTO V_ID_PROVINCIA, V_NOMBRE;
        EXIT WHEN FIDE_DIRECCION_PROVINCIA_TB%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(' La provincia es : ' || V_NOMBRE || ' ( y su Id es : ' || V_ID_PROVINCIA || ')');
    END LOOP;
    CLOSE FIDE_DIRECCION_PROVINCIA_TB;
END;
/
