...
BEGIN
  ...
  IF V_RESULTADO IS NULL OR V_RESULTADO = '' THEN
    RETURN 'NO SE ENCONTRARON MASCOTAS PARA EL DUENNO ' || P_IDENTIFICACION_USUARIO;
  ELSE
    RETURN RTRIM(V_RESULTADO, ', ');
  END IF;
END;
/
--Obtener cirugano por ID DE CIRUGIA
CREATE OR REPLACE FUNCTION OBTENER_DOCTOR_CIRUGIA_FN(
  P_ID_CIRUGIA NUMBER
) 
RETURN VARCHAR2 IS 
  V_CIRUJANO VARCHAR2(100); 
BEGIN 
  SELECT MEDICO_CIRUJANO 
  INTO V_CIRUJANO 
  FROM FIDE_ROLES_CIRUGIA_TB 
  WHERE ID_ROL_CIRUGIA_PK = P_ID_CIRUGIA; 
  RETURN V_CIRUJANO;
EXCEPTION 
  WHEN NO_DATA_FOUND THEN 
    RETURN 'NO SE ENCONTRO CIRUJANO PARA LA OPERACION ' || P_ID_CIRUGIA;
  WHEN OTHERS THEN 
    RETURN 'ERROR AL OBTENER EL CIRUJANO';
END;
/
